...
(function ($) {
	...
	function updatePost(){
		var count = document.getElementById('post-container').getAttribute('data-count');
		$.ajax({
			url: 'site/check-post/'+count,
			type: 'GET',
			success: function(response) {
				if(response.status == 'OK'){
					
					$('#post-container').html(response.html);
					$('#post-container').attr('data-count', response.count);
					reloadJsImage();
			...
			error: function (err) {
				console.log('Error in check post');
				console.log(err);
			}
		})
	}
